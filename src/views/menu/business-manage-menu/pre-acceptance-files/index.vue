...
import ModifiNum from '@/views/menu/business-manage-menu/pre-acceptance-files/components/modifi_num.vue'
import LookProject from '@/views/menu/business-manage-menu/pre-acceptance-files/components/look_project.vue'
export default {
  name: 'pre-acceptance-files',
  components: {
    ModifiNum,
    LookProject
  },
  ...
  setup(prop, context) {
    let tableList = reactive({
      ...
      thead: [
        ...
        { label: '工程地点', prop: 'pro_site', checked: false, width: '200' },
        ...
        { label: '拟竣工日期', prop: 'completed_date', checked: true, width: '200' },
        { label: '录入人', prop: 'enter_pepole', checked: false, width: '100' },
        { label: '录入时间', prop: 'enter_time', checked: false, width: '150' },
        { label: '建筑面积(㎡)/建设规模', prop: 'construct_area', checked: false, width: '200' },
        { label: '工程造价', prop: 'pro_cost', checked: false, width: '200' },
    ...
    const fun_create_project = () => {
      context.root.$router.push({ path: '/business-manage/pre-acceptance-files/increase-files' })
    }

    const fun_modify_project = row => {
      context.root.$router.push({
        path: '/business-manage/pre-acceptance-files/modify-files',
        query: {
          id: row.id
        }
      })
