  ...
  <d2-container type="card" better-scroll :breadcrumb="breadcrumb">
    ...
    <template>
      ...
      <div v-if="look_handover_obj_show">
        <look-handover
          :look_handover_obj="look_handover_obj"
          @fun_look_handover_close="fun_look_handover_close"
        />
      </div>
    </template>
  </d2-container>
...
import LookHandover from '@/views/menu/business-manage-menu/business-queries/components/look_handover_certificate.vue'
export default {
  ...
  setup(prop, context) {
    let tableList = reactive({
      ...
      thead: [
        ...
        { label: '发证日期', prop: 'certificate_date', checked: true, width: '200' },
        ...
        { label: '工程地址', prop: 'eng_address', checked: false, disabled: true, width: '200' },
        ...
        { label: '开工日期', prop: 'ment_date', checked: false, width: '200' },
        { label: '竣工日期', prop: 'ple_date', checked: false, width: '200' },
        { label: '总投资(万元)', prop: 'invest_total', checked: false, width: '200' },
        { label: '建筑面积(m²)', prop: 'floor_area', checked: false, width: '200' },
        { label: '结构类型', prop: 'struct_type', checked: false, width: '200' },
        { label: '案卷总数', prop: 'case_file_total', checked: false, width: '200' },
        { label: '文字材料(卷)', prop: 'word_v', checked: false, width: '200' },
        { label: '图纸(卷)', prop: 'draw_v', checked: false, width: '200' },
        { label: '照片(张)', prop: 'photo_z', checked: false, width: '200' },
        { label: '录像带(盒)', prop: 'video_b', checked: false, width: '200' },
        ...
        { label: '录入人', prop: 'hen_date', checked: false, width: '200' },
        { label: '录入时间', prop: 'test_field', checked: false, width: '200' },
        {
          label: '操作',
          prop: 'operation',
          type: 'operation',
