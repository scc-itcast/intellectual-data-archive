  ...
  setup(prop, context) {
    let tableList = reactive({
      ...
      thead: [
        ...
        { label: '销毁监督人员', prop: 'destroy_super_people', checked: true, width: '100' },
        { label: '销毁事由', prop: 'destroy_reason', checked: true, width: '120' },
        { label: '销毁结果', prop: 'destroy_results', checked: true, width: '120' },
        { label: '销毁状态', prop: 'destroy_status', checked: true, width: '120' },
    ...
    let contextData = reactive({
      ...
      breadcrumb: [
        ...
        { path: '/arch-ident-dest/arch-dest', title: '档案销毁' }
      ],
      queryList: {
      ...
      destroy_regist_status_list: [
        { label: '销毁登记', prop: 'destroy_regist' },
        { label: '销毁完成', prop: 'destroy_complete' }
      ],
      advanced_query_obj: {
    ...
    onMounted(async () => {
      // 调用方法, 方法里调用接口
      contextData.queryList.destroy_regist_status.list = contextData.destroy_regist_status_list
    })

    const fun_value_change = obj => {
      let { value, index } = obj
      contextData.queryList[index].value = value
      if (index === 'destroy_regist_status') {
        fun_destroy_regist_status(obj)
      }
    }

    const fun_destroy_regist_status = obj => {
