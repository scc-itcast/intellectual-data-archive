  ...
  <d2-container type="card" better-scroll :breadcrumb="breadcrumb">
    ...
    <template>
      ...
      <div class="table-list--box"> 
        <sz-table :config="tableList" @fun_db_click="fun_db_click" @fun_selection_change="fun_selection_change"></sz-table>
      </div>
...
import LookProject from '@/views/menu/file-bgf-menu/file-box-manage/components/look_project.vue'
export default {
  name: 'file-box-manage',
  components: {
    LookProject
  },
  ...
  setup(prop, context) {
    let tableList = reactive({
      ...
      thead: [
        { label: '盒名称', prop: 'box_name', checked: true, disabled: true, width: '200' },
        { label: '盒档号', prop: 'box_no', checked: true, disabled: true, width: '200' },
        { label: '盒编号', prop: 'box_num', checked: true, disabled: true, width: '200' },
        { label: '编制单位', prop: 'compile_unit', checked: true, width: '200' },
        ...
        { label: '存放地址', prop: 'store_address', checked: false, width: '200' },
        { label: '背脊宽度', prop: 'spine_width', checked: true, disabled: true, width: '200' },
        { label: '盒整理人', prop: 'box_tidy_p', checked: false, width: '200' },
        { label: '所属单元格', prop: 'own_cell', checked: false, width: '200' },
        { label: '盒内案卷数', prop: 'files_box_num', checked: false, width: '200' },
        { label: '编制起始日期', prop: 'prepare_date_first', checked: false, width: '200' },
        { label: '编制终止日期', prop: 'prepare_date_second', checked: false, width: '200' },
        {
          label: '操作',
          prop: 'operation',
          checked: true,
    ...
    let contextData = reactive({
      ...
      breadcrumb: [
        ...
        { path: '/file-bgf/file-box-manage', title: '档案盒管理' }
      ],
      queryList: {
        box_name: { title: '盒名称', value: '' },
        box_no: { title: '盒档号', value: '' }
      },
      button_list: {
    ...
    const fun_create_project = () => {
      context.root.$router.push({ path: '/file-bgf/file-box-manage/increase-files-box' })
    }

    const fun_modify_project = row => {
      context.root.$router.push({
        path: '/file-bgf/file-box-manage/modify-files-box',
        query: {
          id: row.id
        }
      })
