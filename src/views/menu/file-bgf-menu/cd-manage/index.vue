...
import LookProject from '@/views/menu/file-bgf-menu/cd-manage/components/look_project.vue'
export default {
  name: 'cd-manage',
  components: {
    LookProject
  ...
  setup(prop, context) {
    let tableList = reactive({
      ...
      thead: [
        {
          label: '移交人',
          prop: 'handed_over_people',
          checked: true,
          disabled: true,
          width: '200'
        },
        { label: '移交日期', prop: 'handed_ove_date', checked: true, disabled: true, width: '200' },
        { label: '光盘编号', prop: 'disc_num', checked: true, disabled: true, width: '200' },
        { label: '存放地址', prop: 'store_address', checked: true, width: '200' },
        { label: '移交单位', prop: 'handed_ove_unit', checked: true, width: '200' },
        { label: '光盘内容', prop: 'disc_content', checked: false, width: '200' },
    ...
    let contextData = reactive({
      ...
      breadcrumb: [
        ...
        { path: '/file-bgf/cd-manage', title: '光盘管理' }
      ],
      queryList: {
        disc_num: { title: '光盘编号', value: '' }
      },
      button_list: {
    ...
    const fun_modify_project = row => {
      context.root.$router.push({
        path: '/file-bgf/cd-manage/modify-cd-regist',
        query: {
          id: row.id
        }
      })
