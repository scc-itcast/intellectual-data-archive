...
export default {
  ...
  actions: {
     ...
     */
    async login({ dispatch }, { login = '', password = '', type = 0 } = {}) {
      // const res = await SYS_USER_LOGIN({ username, password })
      const { data } = await SYS_USER_LOGIN({ login, password, type })
      ...
      util.cookies.set('uid', data.uid)
      util.cookies.set('access_token', data.access_token)
      util.cookies.set('partner_id', data.partner_id)
      // 设置 vuex 用户信息
