<template>
  <div id="header-user">
    <el-dropdown size="small" class="d2-mr" @visible-change="fun_visible_change">
      <!-- trigger="click" -->
      <div class="btn-text">
        <div v-if="info.name" class="btn-text-group">
          ...
          {{ info.name }}
          <template>
            <span v-if="visible_change" class="btn-triangle-bottom"></span>
            <span v-else class="btn-triangle-top"></span>
          </template>
        </div>
...
export default {
  ...
  methods: {
    ...
    fun_visible_change(boolen) {
      this.visible_change = boolen
    }
  }
}
</script>
